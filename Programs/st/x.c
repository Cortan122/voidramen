...
static void incrementAlpha(const Arg *);
...
static char* backgroundColorName = NULL;
static float defaultAlpha = -1;
...
xloadcolor(int i, const char *name, Color *ncolor)
{
  ...
  if(i == defaultbg && backgroundColorName != name){
    if(backgroundColorName)free(backgroundColorName);
    backgroundColorName = strdup(name);
  }
...
void updateAlpha(){
  if(defaultAlpha == -1)defaultAlpha = alpha;
...
void incrementAlpha(const Arg * a){
  if(a->i == 0 && defaultAlpha != -1)alpha = defaultAlpha;
  else alpha += a->f;
  if(alpha > 1)alpha = 1;
  if(alpha < 0)alpha = 0;
  if(backgroundColorName)xsetcolorname(defaultbg, backgroundColorName);
  updateAlpha();
  redraw();
}
...
xsetcolorname(int x, const char *name)
{
  ...
  if(x == defaultbg && name[0] == '['){
    const char* realName = name;
    while(name[0] && name[0] != ']')name++;
    if(!name[0]){
      name = realName;
    }else{
      alpha = strtof(realName+1, NULL);
...
xhints(void)
{
	...
	sizeh->height = win.h;
	sizeh->width = win.w;
	sizeh->height_inc = 1;//win.ch;
	sizeh->width_inc = 1;//win.cw;
